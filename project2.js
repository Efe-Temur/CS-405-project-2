...
class MeshDrawer {
	...
	constructor() {
		 ...
		 */
		this.lightPos = [lightX, lightY, 1.0];  // Initial light position
		this.ambient = 0.2;  // Initial ambient light intensity
	...
	draw(trans) {
		...
		gl.uniform1f(gl.getUniformLocation(this.prog, 'ambient'), this.ambient);
		gl.uniform1i(gl.getUniformLocation(this.prog, 'enableLighting'), this.enableLighting);
...
const meshFS = `
			...
			void main()
			{
				if(showTex && enableLighting){
					...
					// Update the fragment color with lighting calculations
